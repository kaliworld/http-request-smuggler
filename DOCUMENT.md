...
The independent `smuggler` command consumes URL lists and, by default, writes
only potential findings as JSON Lines to `comeout.txt`. The `-all` option emits
clean targets and failures as well. It is built entirely from the Go standard library plus repository
