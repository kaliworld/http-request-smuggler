...
func main() {
	...
	output := flag.String("output", "comeout.txt", "JSON Lines vulnerability report file, or - for stdout")
	reportAll := flag.Bool("all", false, "include non-vulnerable and failed targets in the report")
	...
	defer cancel()
	if err := run(ctx, in, out, *concurrency, batch.Options{Timeout: *timeout, Techniques: techniques}, *reportAll); err != nil {
		fatal(err.Error())
	}
}

func run(ctx context.Context, input io.Reader, output io.Writer, concurrency int, options batch.Options, reportAll bool) error {
	...
	for result := range results {
		// The default report is actionable: connection failures and clean targets
		// are omitted, while every potential finding retains its full evidence.
		if !reportAll && !result.PotentialIssue {
			continue
		}
